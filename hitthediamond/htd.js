function HitTheDiamond(selector) {
    ...
    function StartGame() {
        ...
        Setup().then(function () { setTimeout(function () { pop(1500); }, 1000); });
    }
    ...
    var instructions = $("<span>").text("Gems will pop out of the doors. If they're not Garnet, Amethyst, Pearl, Peridot, or Lapis Lazuli, click on them to knock them back down.");
    instructions.append($("<ul>").append(gems.map(function (gem) {
        return $('<li>').text(gem.name + ": " + gem.score + " points");
    })));
    var gameovertext = "Game Over. Score: %S. %C!";
    ...
    function GameOver() {
        ...
        Dialog(gameovertext.replace('%S', score).replace('%C', "Yay"), "Play Again", "Credits").then(function (s) { return s; }, function (f) {
